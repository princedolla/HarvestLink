...
const HomePage = () => {
  ...
  const googleTranslateLoaded = useRef(false);
  ...
  useEffect(() => {
    ...
    const initializeGoogleTranslate = () => {
      if (googleTranslateLoaded.current) return;
      googleTranslateLoaded.current = true;
      ...
      try {
        ...
        // Set up listener for language changes from the widget
        const setupLanguageListener = () => {
          ...
          if (select) {
            ...
            // Set initial language to Kinyarwanda
            setTimeout(() => {
              changeLanguage("rw");
            }, 500);
        ...
        // Try to set up listener, retry if not ready
        if (!setupLanguageListener()) {
    ...
    // Cleanup function
    return () => {
      // Remove any global callback to prevent memory leaks
  ...
  const changeLanguage = (lang) => {
    ...
    try {
      ...
      // Also trigger the Google Translate change
      if (window.google && window.google.translate) {
        window.google.translate.translatePage(lang);
      }
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-gray-900/60 backdrop-blur-md rounded-2xl p-8 border border-gray-700/50 shadow-2xl shadow-black/50">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link
                  to="/login"
                  className="px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:from-green-700 hover:to-emerald-700 transition-all duration-300 shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
                >
                  <FontAwesomeIcon icon={faSignInAlt} /> Login
                </Link>
                <Link
                  to="/register"
                  className="px-6 py-3 bg-gray-800 text-green-400 rounded-lg hover:bg-gray-700 transition-all duration-300 border border-gray-700 hover:border-green-400 flex items-center justify-center gap-2"
                >
                  <FontAwesomeIcon icon={faUserPlus} /> Register
                </Link>
              </div>
      ...
      <section className="py-16 bg-gradient-to-b from-gray-950 to-gray-900">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div className="bg-gray-800/50 p-6 rounded-xl border border-gray-700/30 hover:border-green-400/30 transition-all duration-300">
